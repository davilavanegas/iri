...
def iri(Y, segment_length, start_pos, step, box_filter = True, method = 2, speed_kph = DEFAULT_SPEED_KPH):
    ...
    if overlap == False:    
        ...
        end_si = np.where(np.isin(YC[:,0], end_X) == True)[0]
        start_si = np.hstack((si,end_si[0:-1]))     
        start_si_o = np.where(np.isin(YC[:,0], start_X_o) == True)[0]
        end_si_o = np.where(np.isin(YC[:,0], end_X_o) == True)[0]
        nsegments = len(end_X)
...
#   YC ... input profile of the iri function
    ...
    segment_lengths = IRI[:,1] - IRI[:,0]
    valid_segments = segment_lengths > 0
    total_distance = IRI[-1,1] - IRI[0,0]
    iri_values = IRI[valid_segments,2]
    iri_weights = segment_lengths[valid_segments]
    average_iri = np.average(iri_values, weights=iri_weights)
    average_iri_std = np.sqrt(np.average((iri_values - average_iri)**2, weights=iri_weights))
